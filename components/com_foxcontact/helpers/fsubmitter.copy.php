...
class FSubmitter extends FDataPump
{
	...
	public function Show()
	{
		...
		$result .= '<input type="hidden" name="' . $this->GetId() . '" value="">';

		switch ($this->Params->get("submittype"))
		{
			case 1:
				// Submit input
				$result .= '<input class="btn btn-success" type="submit" value="' . $this->Params->get("submittext") . '"/>' . PHP_EOL;
				break;

			default:
				// Submit button
				$icon = $this->Params->get("submiticon");
				$result .= '<button class="btn btn-success" type="submit">' . PHP_EOL .
					'<span ';
