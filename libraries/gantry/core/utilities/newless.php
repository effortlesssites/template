 ...
 *
 * LESS CSS compiler, adapted from http://GantryLessCompilerss.org
 *
 ...
 * formatter, like `GantryLessCompiler_formatter` which then outputs CSS as a string.
 *
 ...
 * In summary:
 *
 * The `GantryLessCompiler` class creates an instance of the parser, feeds it LESS code,
 ...
 *
 * The `GantryLessCompiler_formatter` takes a CSS tree, and dumps it to a formatted string,
...
class GantryLessCompiler_parser {
	...
	public function __construct($GantryLessCompiler, $sourceName = null) {
		$this->eatWhiteDefault = true;
		// reference to less needed for vPrefix, mPrefix, and parentSelector
		$this->GantryLessCompiler = $GantryLessCompiler;
			...
			if (is_null($block->type)) {
				...
				if (!isset($block->args)) {
					foreach ($block->tags as $tag) {
						if (!is_string($tag) || $tag{0} != $this->GantryLessCompiler->mPrefix) {
							$hidden = false;
							break;
						}
					}
	...
	protected function fixTags($tags) {
		...
		foreach ($tags as &$tag) {
			if ($tag{0} == $this->GantryLessCompiler->vPrefix)
				$tag[0] = $this->GantryLessCompiler->mPrefix;
		}
		return $tags;
	}
			...
			while (true) {
				...
				if ($this->string($str)) {
					...
					foreach ($str[2] as &$chunk) {
						$chunk = str_replace($this->GantryLessCompiler->parentSelector, "$&$", $chunk);
					}
	...
	protected function variable(&$name) {
		$s = $this->seek();
		if ($this->literal($this->GantryLessCompiler->vPrefix, false) &&
			($this->variable($sub) || $this->keyword($name)))
		{
			if (!empty($sub)) {
				$name = array('variable', $sub);
			} else {
				$name = $this->GantryLessCompiler->vPrefix.$name;
			}
			return true;
		}
